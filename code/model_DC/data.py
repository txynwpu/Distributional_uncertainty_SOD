...
class test_dataset:
    ...
    def load_data(self, gen_cor_flag = False):
        if gen_cor_flag:
            corruptimg = self.rgb_loader(self.images[random.randint(0, self.size - 1)])
            corruptimg = self.transform(corruptimg).unsqueeze(0)
            return corruptimg
        else:
...
class EvalDataset(Dataset):
    def __init__(self, img_root, gt_root, testsize, filelist):
        self.img_root = img_root
        self.gt_root = gt_root
        self.testsize = testsize
        self.test_filelist = self._read_filelist(filelist)
        self.transform = transforms.Compose([
            transforms.Resize((self.testsize, self.testsize)),
            transforms.ToTensor(),
            transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])

    def _read_filelist(self, filelist):
    ...
    def __len__(self):
        # return len(self.test_filelist)
        return 100

    def __getitem__(self, index):
        img_name, gt_name = self.test_filelist[index]
