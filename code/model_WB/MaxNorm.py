...
class MaxNorm_via_PGD():
    ...
    def setPerLayerThresh(self, model):
        ...
        for curLayer in [model.decoder.output_conv[4].weight, model.decoder.output_conv[4].bias]:  # here we only apply MaxNorm over the last two layers
    ...
    def PGD(self, model):
        ...
        for i, curLayer in enumerate([model.encoder.fc.weight,
                                      model.encoder.fc.bias]):  # here we only apply MaxNorm over the last two layers
            curparam = curLayer.data
