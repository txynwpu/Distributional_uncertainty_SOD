...
save_path = '/data/local_userdata/tianxinyu/CVPR_2023/Models_distribution/model_deep_ensemble/'
...
for epoch in range(1, (opt.epoch+1)):
    ...
    for i, pack in enumerate(train_loader, start=1):
        for rate in size_rates:
            ...
            pred1, pred2, pred3, pred4, pred5 = generator(images)
            loss1 = structure_loss(pred1, gts)
            loss2 = structure_loss(pred2, gts)
            loss3 = structure_loss(pred3, gts)
            loss4 = structure_loss(pred4, gts)
            loss5 = structure_loss(pred5, gts)
            loss_all = loss1 + loss2 + loss3 + loss4 + loss5
            # loss_r = 0
            # for parameter in generator.parameters():
            #     loss_r += torch.sum(parameter ** 2)
            # loss_all = loss_all + opt.weight_decay * loss_r
...
for dataset in test_datasets:
    ...
    for i, (image, gt, HH, WW, name) in enumerate(tqdm(test_loader)):
        ...
        mean_pred = torch.mean(pred_cat, 1, keepdim=True)
        mean_pred = F.upsample(mean_pred, size=[HH, WW], mode='bilinear', align_corners=False)
        mean_pred = (mean_pred - mean_pred.min()) / (mean_pred.max() - mean_pred.min() + 1e-8)
        res = (255 * mean_pred).data.cpu().numpy().squeeze()
        cv2.imwrite(save_path_pred + name, res)

        uncertainty = torch.var(pred_cat, 1, keepdim=True)
        uncertainty = F.upsample(uncertainty, size=[HH, WW], mode='bilinear', align_corners=False)
